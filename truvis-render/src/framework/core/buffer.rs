...
impl RhiBuffer
{
    ...
    ) -> Self
    {
        ...
        unsafe {
            ...
            let (buffer, allocation) = if let Some(offset_align) = min_align {
                rhi.vma()
                    .create_buffer_with_alignment(&buffer_info, &alloc_info, offset_align)
                    .unwrap()
            } else {
    ...
    #[inline]
    pub fn new_accleration_scratch_buffer<S: AsRef<str>>(
        size: vk::DeviceSize,
        debug_name: S,
    ) -> Self
    {
    ...
    {
        unsafe {
            Rhi::instance().device().get_buffer_device_address(
