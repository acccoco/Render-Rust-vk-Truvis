...
impl ObjLoader {
    fn load(obj_file: impl AsRef<str>) {
        let (models, materials) =
            tobj::load_obj(obj_file.as_ref(), &tobj::LoadOptions::default()).expect("Failed to OBJ load file");
        ...
        for (i, m) in models.iter().enumerate() {
            ...
            println!("model[{}].indices_count       = {}", i, mesh.indices.len());
            ...
            for face in 0..mesh.face_arities.len() {
                ...
                if !mesh.texcoord_indices.is_empty() {
                    ...
                    println!(" face[{}].texcoord_indices = {:?}", face, texcoord_face_indices);
                }
                if !mesh.normal_indices.is_empty() {
                    let normal_face_indices = &mesh.normal_indices[next_face..end];
                    println!(" face[{}].normal_indices   = {:?}", face, normal_face_indices);
                }
            ...
            // this example.
            println!("model[{}].positions        = {}", i, mesh.positions.len() / 3);
        ...
        for (i, m) in materials.iter().enumerate() {
            ...
            if let Some(ambient) = m.ambient {
                println!("    material.Ka = ({}, {}, {})", ambient[0], ambient[1], ambient[2]);
            }
            if let Some(diffuse) = m.diffuse {
                println!("    material.Kd = ({}, {}, {})", diffuse[0], diffuse[1], diffuse[2]);
            }
            if let Some(specular) = m.specular {
                println!("    material.Ks = ({}, {}, {})", specular[0], specular[1], specular[2]);
            }
...
mod test {
    ...
    #[test]
    fn load_obj() {
        ObjLoader::load("D:/code/Render-Rust-vk-Truvis/assets/obj/spot.obj")
    }
}
