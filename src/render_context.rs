...
impl RenderContext
{
    ...
    pub fn acquire_frame()
    {
        ...
        {
            let mut cmd = Self::alloc_command_buffer(format!("ctx-1st-color-layout-trans-frame-{}", ctx.frame_index));
    ...
    pub fn submit_frame()
    {
        ...
        {
            let mut cmd = Self::alloc_command_buffer(format!("ctx-2nd-color-layout-trans-frame-{}", ctx.frame_index));
            cmd.begin(vk::CommandBufferUsageFlags::ONE_TIME_SUBMIT);
            cmd.image_barrier(
...
mod _ctx_init
{
    ...
    use crate::{
        render_context::{RenderContext, RENDER_CONTEXT},
