...
use crate::{rhi::Rhi, rhi_type::buffer::RhiBuffer};
...
impl RhiImage2D
{
    pub fn new(image_info: &vk::ImageCreateInfo, alloc_info: &vk_mem::AllocationCreateInfo, debug_name: &str) -> Self
    {
    ...
    pub fn transfer_data(&mut self, data: &[u8])
    {
        ...
        let mut stage_buffer =
            RhiBuffer::new_stage_buffer(std::mem::size_of_val(data) as vk::DeviceSize, "image-stage-buffer");
        stage_buffer.map();
