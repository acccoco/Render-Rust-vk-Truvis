...
pub struct RhiMemAllocator {
    inner: vk_mem::Allocator,
}

impl RhiMemAllocator {
    /// 由于 vma 的生命周期设定：需要引用 Instance 以及 Device，并确保在其声明周期之内这两个的引用是有效的.
    /// 因此需要在 Rhi 的其他部分都初始化完成后再初始化 vma，并确保 Instance 和 Device 是 Pin 的
...
impl Deref for RhiMemAllocator {
