...
impl RhiRenderingInfo {
    pub fn new(
        ...
        Self {
            color_attach_info: color_attach_image.iter().map(|view| FrameBuffer::get_color_attachment(*view)).collect(),
            depth_attach_info: depth_attach_image.map(FrameBuffer::get_depth_attachment),
            range,
        }
    }
