...
use crate::{
    foundation::{debug_utils::RhiDebugType, device::RhiDeviceFunctions},
    render_context::Rhi,
};
...
impl RhiFence
{
    ...
    pub fn new(render_context: &Rhi, signaled: bool, debug_name: &str) -> Self
    {
        let fence_flags = if signaled { vk::FenceCreateFlags::SIGNALED } else { vk::FenceCreateFlags::empty() };
        let fence = unsafe {
            render_context.device().ash_handle().create_fence(&vk::FenceCreateInfo::default().flags(fence_flags), None).unwrap()
        };

        let fence = Self {
            fence,
            device: render_context.device().functions.clone(),
        };
        render_context.debug_utils().set_debug_name(&fence, debug_name);
        fence
    }
...
impl RhiSemaphore
{
    pub fn new(render_context: &Rhi, debug_name: &str) -> Self
    {
        let semaphore =
            unsafe { render_context.device().ash_handle().create_semaphore(&vk::SemaphoreCreateInfo::default(), None).unwrap() };

        let semaphore = Self {
            semaphore,
            device: render_context.device().functions.clone(),
        };
        render_context.debug_utils().set_debug_name(&semaphore, debug_name);
        semaphore
    }

    pub fn new_timeline(render_context: &Rhi, initial_value: u64, debug_name: &str) -> Self
    {
        ...
        let semaphore = unsafe { render_context.device().ash_handle().create_semaphore(&timeline_semaphore_ci, None).unwrap() };
