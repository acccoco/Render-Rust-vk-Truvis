...
struct DataConvert
{
    ...
    static CxxVec2f vec2(const aiVector3D& vec) { return CxxVec2f{.x = vec.x, .y = vec.y}; }

    /// 将 Assimp 的颜色转换为 C_Vec4f
    static CxxVec4f vec4(const aiColor4D& color)
    {
        return CxxVec4f{.x = color.r, .y = color.g, .z = color.b, .w = color.a};
    }
